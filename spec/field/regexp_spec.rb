...
require "mem_db/regexp_engines/re2"

RSpec.describe MemDB::Field::Regexp do
  let(:field) { described_class.new(:text, rx_engine: rx_engine, ignore_case: ignore_case) }
  let(:ignore_case) { false }

  shared_examples "regexp" do
    ...
    context "when text case mismatch" do
      ...
      context 'when ignore_case: true' do
  ...
  context 'when standard ruby regexp' do
    let(:rx_engine) { MemDB::RegexpEngines::Std }

    it_behaves_like "regexp"
  end

  context 'when re2 regexp engine used' do
    let(:rx_engine) { MemDB::RegexpEngines::Re2 }
